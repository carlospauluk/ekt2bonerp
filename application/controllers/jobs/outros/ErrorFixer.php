...
class ErrorFixer extends CI_Controller
{
    ...
    {
        $r = $this->dbcrosier->query("SELECT distinct table_name, column_name, column_type FROM information_schema.columns WHERE table_schema = 'crosier' AND table_name NOT LIKE 'vw%' and column_type LIKE '%date%'")->result_array();
        
        foreach ($r as $l) {
            $dtR = $this->dbcrosier->query("SELECT 1 FROM " . $l['table_name'] . " WHERE DATE_FORMAT(" . $l['column_name'] . ",'%Y-%m-%d') = '0000-00-00'")->result_array();
            if (count($dtR) > 0) {
